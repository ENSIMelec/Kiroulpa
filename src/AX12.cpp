...
int main(int argc, char **argv) {
    ...
    do {
        ...
        dxl_comm_result = packetHandler->read2ByteTxRx(portHandler, DXL_ID, ADDR_AX_PRESENT_POSITION,
        ...
        dxl_comm_result = packetHandler->write2ByteTxRx(portHandler, DXL_ID, ADDR_AX_GOAL_POSITION,
                                                        dxl_present_position, &dxl_error);
        if (dxl_comm_result != COMM_SUCCESS) {
            cout << "Erreur COMM ecriture angle " << DXL_ID << endl;
        } else if (dxl_error != 0) {
        ...
        dxl_comm_result = packetHandler->read2ByteTxRx(portHandler, DXL_ID, ADDR_AX_PRESENT_LOAD, &dxl_present_load,
                                                       &dxl_error);
        if (dxl_comm_result != COMM_SUCCESS) {
    ...
    dxl_comm_result = packetHandler->write1ByteTxRx(portHandler, DXL_ID, ADDR_AX_TORQUE_ENABLE, TORQUE_DISABLE,
                                                    &dxl_error);
    if (dxl_comm_result != COMM_SUCCESS) {
